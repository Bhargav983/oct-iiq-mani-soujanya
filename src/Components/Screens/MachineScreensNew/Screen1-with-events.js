        ...
        if (data.data?.length > 0) {
          ...
          if (deviceData) {
            ...
            setSensorData({
              ...
              errorCode: deviceData.error_code?.value || "0",
              isOnline: (() => {
    try {
      const activePcb = (typeof activePCBRef !== 'undefined' && activePCBRef?.current) || 
                        (typeof selectedService !== 'undefined' && selectedService?.pcb_serial_number) || 
                        (typeof sensorData !== 'undefined' && sensorData?.deviceId);
      if (activePcb) {
        const stored = localStorage.getItem('heartbeat_' + activePcb);
        if (stored) {
          const parsed = JSON.parse(stored);
          if (parsed.device_status === 'OFFLINE') return false;
          if (parsed.device_status === 'ONLINE') return true;
        }
      }
    } catch (e) {}
    return isOnline;
